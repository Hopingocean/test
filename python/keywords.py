        ...
        for B_start in range(1, A_end + 1):
            ...
            if (min(A_times, B_times) * thresholdMin <= wordTimes
                ...
                if A_end > B_start:
                    ...
                    if debug:
                        print('repeat: %s / %s (%s->%s)' % (repeat, word, wdDict[len(repeat)][repeat], wdDict[len(repeat)][repeat] + wordTimes))
                ...
                if debug:
                    print('==合体成功: %s=%s, %s=(%s->%s), %s=(%s->%s)\n' %
                          (word, wordTimes, A_word, A_times, wdDict[len(A_word)][A_word], B_word, B_times, wdDict[len(B_word)][B_word]))
...
def remove_neighbor(mainWdLen, wdDict, word, nbrDict, debug):
    for higherLength in range(mainWdLen + 1, mainWdLen * 2):
        for higherWd in wdDict.get(higherLength, {}):
            ...
            # word开头或结尾
            if word in [higherWd[:mainWdLen], higherWd[-mainWdLen:]]:
                # 获取相邻词 仅替换一次
                nbr = ((higherWd[:mainWdLen] + higherWd[-mainWdLen:]).replace(word, '', 1))
                N_Times = wdDict[len(nbr)][nbr]  # 邻词原次数
                removed = nbrDict.get(nbr, 0)  # 已被移除过的次数
                # 更新邻词字典
                nbrDict[nbr] = max(wdDict[len(higherWd)][higherWd], removed)
                # 不出现负数
                wdDict[len(nbr)][nbr] -= (min(nbrDict[nbr] - removed, wdDict[len(nbr)][nbr]))
                if debug:
                    print('# %s=%s->%s,上级:%s=%s' % (nbr, N_Times, wdDict[len(nbr)][nbr], higherWd, wdDict[len(higherWd)][higherWd]))
...
def detectKeywords(content, timesLimit=10, wordLenMin=2, wordLenMax=5,
    ...
    wdDict = combineWords(wdDict, timesLimit, thresholdMin, thresholdMax, debug)
    checkResult(wdDict, content)
    keywordsList = printResult(asciiDict, wdDict, wordLenMin, timesLimit, topLimit, doPrint, letter=letter, digit=digit)  # 最小出现次数
    return keywordsList
